...
- **Long tasks under a token/session budget — checkpoint, don't trust partial fan-outs.** (1) **Commit each verified unit to the branch immediately** — git survives a session cut-off; never hold a big multi-step result only in the conversation. (2) **Size a fan-out to the budget before launching** (agents × ~40–60k tokens); tight budget → fewer/leaner agents or phases you read between; prefer one well-scoped fan-out per turn over launching everything. (3) **A partial subagent run ≠ a clean one:** always check `agents_error`/`failures`/`unverified` — a run where agents died on a session-limit is INCOMPLETE, never tag it ✅/verified/done; workflows must RETURN what they couldn't verify, not silently drop it. (4) **Recover right:** the limit resets on a clock → wait, then re-run; `resumeFromRunId` (cache) only if code+prompts are unchanged — after applying fixes, run a FRESH review on the new HEAD (stale findings reference dead code). (5) **Fail loud:** say when a result is partial + the recovery plan. Details: `docs/architecture/GOTCHAS.md`.
