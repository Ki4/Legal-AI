# Project Changelog — Archive (2026 H1, sessions ≤49)

> Архів старих why-записів, перенесений з `apps/client/.claude/changelog.md` (session 52, 2026-06-29)
> заради гігієни контексту: живий changelog читається на старті кожної сесії (`/session-start`), тож
> тримаємо в ньому лише останні ~3 сесії. Цей файл **не читається** автоматично — `grep` за потреби
> («чому ми зробили X місяці тому»). Append-only; нове додавати зверху при наступному архівуванні.
...
**What happened:**
- Закрито **#3** [#36] (valid_from/valid_to) — безпосередній ризик «генерація за застарілою редакцією» вже закрито переглянутим `is_stale`-підходом (#29, issue #11, s.34); темпоральне версіонування → відкладено до GraphRAG (#36/#46).
- Закрито **#8** [#27] (prompt versioning) — застаріло після doc-engine (#34): генерація з `document_template`, `ai_prompt` декоративний; LLM-крок склонення → #54 eval.
- Закрито **#23** [#5] (монетизація lawyer_type/revenue_share) — рішення Сергія: тримати як ідею в IMPROVEMENTS #5 (revenue-модель переглядається в strategy_v2), не як активний issue.
- Лишено **12 відкритих** (актуальні + окремі): #5,#10,#13,#15,#16,#19,#20,#21,#22,#24,#26. #22 (портфоліо) — свідомо (LinkedIn/job-search Сергія, не продукт).
- IMPROVEMENTS #36/#27/#5 анотовано статусом, щоб ідеї пережили закриття issue.
**Files:** `docs/architecture/IMPROVEMENTS.md` (#36/#27/#5 статус-пометки).
**Tests:** none (doc + issue-метадані).
**Note:** Гілка #67 (`fix/divorce-property-debt-variant-b`) з логом session 43 лишається held під Olga — її changelog/summary-записи приїдуть у main при мерджі тієї гілки. **(→ змержено session 50, `d68a92b`.)**

### 2026-06-21 (session 43) — divorce: майно/борги без порожніх плейсхолдерів (Variant B, #67)
**Status:** COMMITTED · branch `fix/divorce-property-debt-variant-b` → **ЗМЕРЖЕНО + задеплоєно session 50 (`d68a92b`), issue #67 ЗАКРИТО** (sign-off Ольги — пост-фактум 1 липня)
...
**Tests:** root vitest **1110/1110** ✅ · client **186/186** ✅ · `tsc -b` clean. **Gate перед проддеплоєм:** Ольга підтверджує формулювання (checklist item 1 issue #67), далі merge + заливка шаблону в Supabase + live form-submit smoke. **(Виконано session 50.)**
