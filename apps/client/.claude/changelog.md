...
**Status:** гілка `feat/style-runs-v2` (A-конвеєр ЗМЕРЖЕНО в main `998f2bc` перед стартом B) · root **1145 ✅** (+31) · UI **468 ✅** (+22) · tsc/eslint clean · build:admin OK · live-верифіковано (включно з реальним кліком) · **n8n deploy PENDING — дозвіл Сергія**
...
**Next:** деплой form-submit (після «да» Сергія) → слайс C (sync-підсвітка каретка↔абзац превʼю) · опційно: mark-декорація в CM, щоб {{#bold}}-діапазон і в редакторі виглядав жирним.
