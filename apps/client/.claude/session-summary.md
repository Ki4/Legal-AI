...
> Updated: 2026-06-29 (session 51 — **law-change-impact G4: дайджест-workflow ЗІБРАНО + ЗАДЕПЛОЄНО live**, гілка `feat/law-change-digest-workflow` (НЕ змержено). Агент «що змінилось» тепер **живий end-to-end**: монітор пише `pending`+`article_diffs` (G1 на main) → workflow `law-change-digest` (id `qTOIqllA4CQvBJs5`, 10 нод, active) робить L2→L3→L4→L5 → юрист бачить `AiDraftCard` (UI вже на main). Генератор з SSoT `scripts/build-law-change-digest.mjs` (анти-дрейф, інлайн scope+groundedness+промпт); self-contained (0 n8n-credentials, секрети через Global Config). **Live verify (webhook, тестовий рядок = реальна ЦПК ст.175 ч.7 #87):** exec 163 `drafted` (обґрунтований, evidence дослівний), 162 `abstained` (RED-гейт спрацював → полагоджено формат diff), 164 порожня черга no-op. Тести **1013 ✅** (+5 guard). Тестовий рядок прибрано, `law_change_log`=0. **🔴 Наступне:** merge гілки (PR для ревью з телефона?) + G5 доки (DECISIONS/IMPROVEMENTS deferred: L4b LLM-критик, поартикульний diff) + опц. GH-Actions kick webhook після монітора. Issue #73. Деталі ↓ Session 51. | session 50 — **велика сесія, ВСЕ в main, main чистий**. 4 одиниці: (1) **declension stem-guard live** (`835d282`); (2) **#67 divorce майно/борги Variant B live + закрито** (`d68a92b`, шаблон залито в Supabase, smoke exec 160 ✅); (3) **law-monitor+CRON верифіковано** (детектор локально+CI однаково; свіжий `workflow_dispatch`=success; ⚠️ `schedule:` лишається OFF, вмикати ПІСЛЯ розрулення 2 змін); (4) **403-фікс** rada WAF (`31a5ed6`, браузерний UA — Про судовий збір знову відстежується) + **diff СК/ЦПК**: СК НЕ материально, **ЦПК ст.175 ч.7 (Закон №4833-IX) материально для alimony** — реквізити рахунку позивача → IMPROVEMENTS #87. **Продуктовий контекст:** 2 послуги = доказ пайплайна+витрина, масштаб = Service Builder (`docs/strategy/where-we-are-and-scaling.md`); GATED-під-Олю робимо самі, sign-off Олі пост-фактум 1 липня. **📋 ЗАВТРА (Сергій тільки з телефоном):** repo-only задачі через cloud-Claude + ревью GitHub mobile — #87 як PR (деплой пізніше), A3 drift-тест; live-n8n заблоковано (десктоп off). Модель червня: Opus+ultra-code. Деталі ↓ Session 50. | session 49 (зведено в session 50): declension stem-guard на гілці — задеплоєно й змержено в session 50. | session 50 деталі ↓ | (попереднє: session 50 — **declension stem-guard ЗАДЕПЛОЄНО live + ЗМЕРЖЕНО в main** (`835d282`): дотиснули 🔴-крок session 49 після того, як Сергій підняв Docker n8n + ngrok. `deploy-workflow.mjs form-submit` → live `D2ab06X3pVUWk1py` (48 нод, active); **3 реальні webhook-прогони** (minimal/divorce-1/alimony-a1) усі `success`; верифіковано з executions API: guard ПРОПУСТИВ валідні AI-форми без хибного відкату — divorce 158 `Петренком Андрієм Сергійовичем`/`Петренко Оксану Іванівну`, alimony 159 `Стягнути з Іванова Івана Івановича на користь Іванової Інни Петрівни`, обидва `_abstained=null`. main чистий. Деталі ↓ Session 50. | session 49 — **declension stem-guard** на гілці `feat/declension-stem-guard` (`06365e5`): детерм. пост-чек над AI-склоненням ПІБ (єдиний LLM-крок) — `render-document.js` нові `wordStemOk/declensionStemOk/guardDeclension` на 4 полях `aiSafe`, LENIENT (відхиляє лише явні mismatch); `Build Document` node регенеровано. Тести: guard **19** + parity **263 байт-у-байт** + n8n/scripts **1008** ✅. Деплой+webhook-тест ВІДКЛАДЕНО (потрібен Docker n8n+ngrok). ⚠️ WebStorm-термінал (JediTerm) не скролить Claude Code TUI — Сергій пробує перезапуск IDE; великі звіти → `.md`-файл. Деталі ↓ Session 49. | session 48 — **block-A hardening, все в main**: A5 заповнений приклад документа в превʼю адмінки (`6f9b042`) + dev-fix `optimizeDeps`; A1 CI тест-гейт `test.yml` (`cdb8491`, PR #72, cloud-прогон ✅); C-EXCEPTION верифіковано — склонення = живий LLM-крок Groq з fallback у називний (`ac245d6`). Модель червня: **Opus + ultra-code**. Чисті DO-NOW вичерпані; далі — declension stem-guard / A3 / GATED-під-Олю. Деталі ↓ Session 48. | session 47 — **#71 ЗМЕРЖЕНО в main (2364e29)**: docx/pdf/html live — divorce/alimony приходять **приватним файлом PDF+DOCX** у Telegram (PII закрито, Google Doc видаляється), верифіковано наживо #154/#155; + превʼю документа в адмінці (G2, таб «Документ») + one-pager «принципи» для демо 1 липня (`docs/strategy/how-it-works-onepager.md`). #67 (divorce) лишається на власній гілці під sign-off Олі. **Наступні кроки → `docs/architecture/backlog-session-47.md`** (наповнений техніками Блоку II §4-7 + Блоку III, з поправкою pressure-test: LLM-харнес DEFERRED, посилюємо детермінований шлях). Раніше в сесії — **Verification Protocol + P1-аудит**: створено `docs/architecture/VERIFICATION-PROTOCOL.md` (status ✅/⚠️/📋/❌ + правило ≥2 згоджених evidence + runtime як окремий клас), P1 anti-hallucination spine звірено на 100% вкл. прод. Ключове: живий шлях divorce/alimony = doc-engine `template` (а репо казало `js`!), захист = детермінізм + checklist-**детектор** (не гейт); критики/RAG/GraphRAG = свідомий future-tier scaffolding (спить); тест-гейта в CI немає. Гілка `claude/affectionate-mccarthy-ho6gqe`. **+ продуктовий розворот:** детермінізм-first, модель блоків A/B/C/D, медицина=пілот (July+); план якісного виводу docx/pdf/html до **1 липня** — issue #71 (Path-1 Google `:export`). **Від Сергія: PR да/ні + Google `:export` scope в n8n.** | session 45 — admin design-system (світла Claude-Design канва + темна тема) + law-change-impact агент (Tier-2, G1-G4, migration 027 застосована) + viz-lab пісочниця; ⚠️ **УСЕ на гілці `claude/wizardly-dirac-qxqw5u`, НЕ змержено в main**, деталі нижче. | session 44 — issue-гігієна: закрито #3/#8/#23 як superseded, решта backlog лишена; деталі нижче. ⚠️ session 43 (divorce #67) тримається held на гілці `fix/divorce-property-debt-variant-b` — її лог приїде в main при мерджі. | session 42 — service-mirror ЗАВЕРШЕНО: слайси 2+3 ЗМЕРЖЕНО в main, migration 025+026 застосовані, слайс 3 e2e-верифіковано наживо (upload→інбокс→signed URL→PDF), **issue #66 ЗАКРИТО**. Знайдено+виправлено передіснуючу поломку `tsc -b` на main.)
