...
**🟢 SESSION 84 (2026-07-13) — Движок автозаповнення юр-документів «з 0» (docxtpl+pymorphy3+Gotenberg): PoC ВАЛІДОВАНО наскрізь → /interview → Tier-2 спека → Фаза A ядра ЗАКОМІЧЕНА+ЗАПУШЕНА. Гілка `feat/doc-engine` (від `docs/cabinet-vision`, НЕ від main).**
- **Що це:** окремий Python-рушій (НЕ чіпає дядин `mcp-server`/`declension.ts`), відповідь на боль «кожна послуга = код, який ми пишемо і верифікуємо». Юрист пише шаблон у Word з `{{полями}}`, рушій підставляє+склоняє+друкує PDF. PoC доведено на ДВОХ документах: простий договір + **реальна позовна заява про аліменти** (канонічний текст узято з `mcp__legal-docs__generate_alimony_document`, адаптовано в Word-шаблон).
- **Доведено прогонами (claim≠fact):** (1) підпис не рветься — `keep_with_next` перевірено В РЕАЛЬНОМУ Gotenberg (не Word); (2) повний пайплайн на договорі; (3) 3-стор. court-ready аліментна заява (умовні блоки розлучення/працевлаштування спрацювали, дати прописом); (4) **склонення pymorphy3 = 1-в-1 з дядиним Groq** на 4 падежах (орудний, родовий чол., родовий жін. з незмінним прізвищем, дитина) — але ЛОКАЛЬНО, офлайн, БЕЗ API. Рушій навіть виправив баг MCP (сира ISO-дата → «1 вересня 2016 року»). Артефакти на робочому столі: `poc-dogovir/`, `poc-alimony/`, `court-docs-ua/` (8 суд.рішень з HF `lang-uk/court-decisions-uk`, MIT — можна показувати партнеру/Олі).
- **Ключове (Word на сервер НЕ ставити):** неможливо (Linux+ліцензія+нестабільність) → PDF друкує Gotenberg (`gotenberg/gotenberg:8`, `/forms/libreoffice/convert`).
- **/interview (medium) → 7 рішень → Tier-2 спека `specs/features/template-doc-engine/`** (requirements+plan+validation, DRAFT): (1) окремий Python-сервіс; (2) шаблон=єдине джерело правди, схема виводиться з нього; (3) юрист САМ виділяє поля (не пише `{{}}`), система генерить ім'я/тип; ветвлення — юрист виділяє блок, AI пропонує умову, юрист підтверджує; (4) склонення=КАСКАД словник→LLM→Оля + пополнювана бібліотека; (5) мульти-док — задел, 1 док реалізувати; (6) критерій готово=parity з MCP + автоперевірки + підпис Олі; (7) авторська панель=розширення `apps/client/src/admin`, НЕ Word-плагін.
- **Фаза A ядра ЗРОБЛЕНА (коміт `d84fa9b`, запушено):** `apps/doc-engine/` — `declension.py` (gender-aware каскад, ніколи не підставляє тихо — закриває DEMO-FINDINGS «B»), `dates.py`, `fill.py`+`preflight.py` (падає на незаповненому полі), `pdf.py` (Gotenberg). **11 тестів зелені** (parity склонення vs MCP + дати, офлайн).
- **🪤 Гочі (в спеці+README, не повторювати):** `{%tr%}` знищує свій рядок → for/endfor в ОКРЕМИХ контрол-рядках, контент між; `StrictUndefined` конфліктує з `{%tr%}` → preflight; pymorphy `parse[0]` без роду схибляє мовчки → gender-aware; керуючі символи ламають docx XML → санітайз.
- **Next (Фаза A хвіст, у свіжому чаті):** `templates/alimony.docx` + `app.py` (FastAPI `POST /generate`) + parity-diff тест vs MCP через Gotenberg + тест пагінації в сервісі. Гілку перед PR перебазувати на main (`git rebase --onto main docs/cabinet-vision feat/doc-engine` або розібратись з untracked `anon-relay-bot-concept.md`).
- **🎯 АЛЕ ОСНОВНИЙ ФОКУС СЕРГІЯ ДО СЕРЕДИ (15.07) — РОЗІБРАТИСЬ У БІЗНЕС-МОДЕЛІ**, не doc-engine. Матеріали вже є: `docs/research/business-model-research-2026-07.md`, `business-model-report-2026-07-share.md`, `competitive-analysis-med-fop-2026-07.md`, `docs/strategy/med-fop-platform-vision.md` (усі untracked на `docs/cabinet-vision`) + memory `project_pivot_med_fop`, `project_strategy_v2`. Doc-engine продовжити ПІСЛЯ.
- **🪤 Контейнер `gotenberg-test` міг лишитись на :3000 — `docker rm -f gotenberg-test` якщо не потрібен.**

**🟢 SESSION 83 (2026-07-11) — Концепція особистого кабінету мед-ФОП («живі документи»): vision-док + кликабельний макет. Гілка `docs/cabinet-vision` (docs-only), чекає ревʼю Сергія → merge.**
- **Що зроблено:** 3 Explore-агенти (клієнт/дані/стратегія) + Plan-агент + інтервʼю Сергія (4 розвилки: платформу НЕ фіксуємо — «спершу визначити, що там має бути» · «Люди» в моделі даних з 1-го дня, UI = простий список без ролей · спільнота = закрита TG-група з анонімністю через бот-проксі + розділ «Питання і пропозиції» · монетизація НЕ фіксується — 3 сценарії A/B/C на обговорення).
- **`docs/strategy/cabinet-vision.md`** — 11 розділів: формула актуальності (ревізія шаблону + снапшот профілю → всі статуси виводяться), цикл «Оксана, стоматолог, 5 працівників» (10 кроків + mermaid state/sequence), статусна модель (чернетка/актуальний/перевіряється/потребує оновлення/застарілий/не потрібен + анти-спам: юзер бачить «потребує оновлення» ЛИШЕ після publish юриста), IA 9 модулів (Головна з кільцем «Готовність до перевірки» на базі 122-чекліста · Документи з історією версій і «Підставою» · Люди · Законодавство · Сповіщення · Підтримка · Спільнота · Профіль з «хвилею» змін · Підписка), workspace-масштабування (соло-ФОП→штат→ТОВ+ролі→клініка БЕЗ ремоделювання), 17 сутностей (✅/🔧/🆕), entitlement=capabilities (матриця A пакет/B підписка/C гібрид-рекомендація), фази MVP→v1.1→v2 (критерій валідації: ≥50% пілотних повертаються + ≥1 живий цикл), ризики, 16 відкритих питань (Олі 1–7, дяді 8–12, ринку 13–16).
- **Ключові рішення:** актуальність = формула, а не флаг · workspace-корінь з 1-го дня · чекліст 122 = дані (хребет дашборда) · **`cases` НЕ ретрофітимо** (нові DocumentInstance/DocumentVersion; 90-дн retention-cron НЕ бачить нові таблиці) · реюз: law-monitor як детектор циклу (без змін), form-движок для онбордингу, `render-document.js`, `service_revisions` (потрібен аддитивний `current_revision_id`), бот-push безкоштовний. GDPR: consent-екран + signed-link-only доставка (бриф з гілки `research/gdpr-med-brief`).
- **`docs/strategy/cabinet-mockup.html`** — самодостатній кликабельний макет (рамка телефону, таб-бар, 11 екранів, дані «Оксана»): кнопка «⚡ Симулювати зміну закону» → push у рамці + дзвіночок +1 + 2 доки «потребує оновлення» + кільце 78→64% → «Оновити документ» → нова версія v3 → кільце 78%. Плюс перемикач монетизації A/B/C на екрані Підписка, «хвиля» профілю (модалка «Це змінить 8 документів»), голосування у «Питання і пропозиції». **Верифіковано playwright наскрізь** (усі екрани, повний цикл, консоль чиста). Це інструмент 5-хв демо для Олі/дяді.
- **Next:** ревʼю Сергія → merge гілки → пройти макет з Олею (питання 1–7) і дядею (8–12) → market-інтервʼю (13–16) → лише потім Tier-2 спека MVP кабінету.
- **🪤 Помічено:** (1) untracked `docs/architecture/anon-relay-bot-concept.md` — НЕ з цієї сесії, лишив як є; (2) session-summary переріс ~3 сесії — архівацію відклав свідомо: гілка s81 (`docs/med-vertical-market-brief`) теж редагує цей файл, рефактор зараз = гарантований конфлікт.
...
**🔴 НАСТУПНА СЕСІЯ (85):**
1. **🎯 ПРІОРИТЕТ ДО СЕРЕДИ 15.07 — БІЗНЕС-МОДЕЛЬ.** Сергій хоче РОЗІБРАТИСЬ у бізнес-моделі (не кодити). Матеріали: `docs/research/business-model-research-2026-07.md` + `business-model-report-2026-07-share.md` + `competitive-analysis-med-fop-2026-07.md` + `docs/strategy/med-fop-platform-vision.md` + memory `project_pivot_med_fop`/`project_strategy_v2`. Розмовний режим, допомогти зрозуміти/зважити монетизацію (A пакет / B підписка / C гібрид).
2. **Doc-engine Фаза A хвіст (ПІСЛЯ бізнес-моделі):** `templates/alimony.docx` + `app.py` FastAPI + parity-diff vs MCP через Gotenberg + тест пагінації. Гілка `feat/doc-engine` запушена, перебазувати на main перед PR.
3. **(Старе, s80-81, ще відкрите)** Post-Оля фідбек на документ, 7 GDPR-питань → go/no-go у DECISIONS.md, sign-off «Список Олі»; merge GDPR-брифу `research/gdpr-med-brief`; ревʼю+merge `docs/cabinet-vision`.
