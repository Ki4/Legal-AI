...
**🟣 WEB-СЕСІЯ (паралельно s62-64) — TEMPLATE-EDITOR: Сесії 1+2 ГОТОВІ на гілці `claude/document-constructor-styling-a3zkky` (top `bd1d848`), чекає ДЕМО-ПРИЙМАННЯ Сергія → потім Сесія 3.**
- **Що це:** редактор шаблона документа для юриста в адмінці (Tier 2, спека
  `specs/features/template-editor/` — requirements/plan/validation; конвеєр 3 сесій; рішення — interview
  hard, 12 питань). Вкладка «📄 Шаблон документа» в `ServiceEditPage`: чернетка→публікація з парс-гейтом
  на реальному `@doc-engine`, снапшот-архів `service_revisions` перед публікацією, тулбар стилів
  (Центр/Праворуч/Жирний/Відступ/З нової сторінки/Тримати разом + скелети `{{#if}}`/`{{#each}}`),
  палітра «Змінні форми» (+ бейджі «не в шаблоні»), режим прев'ю «Показати змінні» (сірі чипи полів,
  digitalved-parity), таби прев'ю Документ/Розкладка по чернетці. Рушій НЕ чіпали.
- **Стан:** UI **406 ✅** (371 база + 35), tsc clean, build:admin OK · БД: draft-колонка ✅ +
  `service_revisions` ✅ (застосовано Сергієм; **🪤 services.id = INTEGER** — див. GOTCHAS) · міграція в
  репо = `031_template_editor.sql` (030 зайняла s63-категорія) · E2E-скелет
  `apps/client/e2e/template-editor.e2e.ts` (Playwright devDep; skip без `E2E_ADMIN_*` env).
- **🔴 НАСТУПНИЙ КРОК — демо-приймання (validation.md):** `npm run dev:admin` → divorce → «Шаблон
  документа»: (1) курсор на дату → «Праворуч» → превʼю зсунуло; (2) «Додатки:» → «З нової сторінки» →
  «Розкладка» переносить; (3) виділити Додатки+підпис → «Тримати разом»; (4) клік поля в палітрі →
  `{{поле}}` у курсор; (5) режим «Показати змінні» → сірі чипи; (6) «Опублікувати» → toast + рядок
  `publish_template` у `SELECT reason, created_at FROM service_revisions ORDER BY created_at DESC LIMIT 3;`;
  (7) негатив: стерти `{{/if}}` → публікація заблокована укр. повідомленням, чернетка зберігається.
- **Після демо — Сесія 3** (спека §5): read-only мітки блоків у превʼю (`detectBlocks` по чернетці) ·
  «Створити з каркаса» (скелет позову, 8 блоків; текст каркаса — sign-off Олі ДО проду) · «Історія змін»
  (список `service_revisions` + «Відновити»). Поза скоупом: UX-злиття View/Edit, блоки Фаза 1-2, WYSIWYG.
- **🪤 Гілку оновлювати через `git fetch && git reset --hard origin/<гілка>`** (був force-push після
  rebase; `git pull` дає хибні конфлікти — вже наступили, розрулено). Фікси по демо робити у WEB-сесії
  (гілка живе там), локально — лише демо/E2E.
