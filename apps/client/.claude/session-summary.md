...
**🟢 SESSION 87 (2026-07-15) — Word → MCP-послуга: новий білдер = КОМПІЛЯТОР. Ключові ідеї + еталон «до надстройки» + каркас спайка. Гілка `feat/word-to-service-poc` (ЗАПУШЕНО, НЕ в main). 43 тести зелені. Спайк ЩЕ НЕ прогнано.**

> ⚠️ **Паралельно йде ДРУГА сесія** на гілці `docs/business-model-med-fop` — вона зайняла номер **86** (бізнес-модель, сегмент соло-медФОП, MVP=самоаудит). Цей трек = **87**. Обидві правлять цей файл на різних гілках → **при мержі буде конфлікт у «Стан зараз», це очікувано**, зливати руками.

**🔑 Ключові ідеї (головне з сесії):**
- **Новий білдер — це КОМПІЛЯТОР, а не редактор форм.** Сергій: «забудь про старий білдер, будуємо новий — з урахуванням переводу послуги з Word у MCP-послугу». Контур: **один `.docx` = єдине джерело** (текст + `customXml`-блоб) → компілятор → `services`-рядок + JSON-схема тула + `watched_laws` + чекліст. Наслідки: (1) **`services`-рядок = ПРОЄКЦІЯ, не джерело** — знести і зібрати заново з `.docx` (звідси когерентне «веб = дзеркало»); (2) **надстройка = редактор контракту**, білдер = компілятор; (3) **per-service скіл = баг, а не фіча**.
- **Мірило амбіції (рішення Сергія):** **послуга №10 додається без жодного рядка нашого коду і без жодного рядка прози у скілі.**
- **Справжня діра — НЕ у Word, а у ШИРИНІ КАНАЛУ.** JSON-схема, яку бачить LLM, **викидає `tab`** (порядок питань) і перетворює `validation` на прозу → **скіл існує рівно щоб компенсувати втрачене** (~150 рядків прози на послугу, пишемо МИ). Порядковий розбір `alimony-claim/SKILL.md`: порядок табів → дані · **межі/відмови послуги → проза ВІД ЮРИСТА** (юридичне судження = авторська робота, не наш код) · формат-лор → `hint` · статті → з цитат у тексті · протокол інтервʼю → **один generic-роутер**. Жоден рядок не мусить бути per-service. **Ставка всієї витівки = розширити канал схеми**; довести **порядковою звіркою ДО** першого рядка надстройки.
- **Подарунок:** юрист **уже пише** «ст. 180 СК України»; `watched_laws` матчиться по канонічному URL; у репо є регулярка цитат + `extract-citations.mjs` → **компілятор збирає `watched_laws` з тексту** → law-monitor (живий, s79) охороняє Word-послугу з народження, БЕЗ SQL-сіду.
- **Що НЕ прийде з Word ніколи (назвати, інакше «ноль коду» = брехня):** (1) вычисляемый слой (склонення/дати = код рушія); (2) **юр-константи** — прожмінімуми 2026 і дріб ст.183 захардкожені в `render-document.js:548` → щорічний тихий дрейф у суді; пропозиція `legal_constants` як дані під охороною law-monitor (окремий трек); (3) Add-in ≠ VBA: нема події «юрист править», не можна блокувати збереження → зіпсований шаблон **ловить КОМПІЛЯТОР**, а не Word (звідси гейти на сервері).

**Стан справ:**
- **Осуществимість = ✅ ТАК** (`docs/architecture/word-addin-feasibility-2026-07-15.md`; звірено з живою докою MS через context7): `getSelection` · `insertText(replace)` · `insertContentControl` · **`customXmlParts.getByNamespaceAsync`** (шукає part ПО NAMESPACE — рівно як `docx_meta.py:31`) — підтверджені. 📋 лишився ОДИН: `paragraph.insertParagraph("Before")` — на ньому висить уся ідея ветвлення.
- **Розвилка A/B (фактом у спайку):** A — літеральні `{%p if %}` (юрист бачить мотлох, може зламати парність) vs **B — Content Control як маркер + jinja впрыскує компілятор** (jinja у документі юриста НЕМА → зламати неможливо). Рекомендація **B**.
- **✅ Еталон «ДО надстройки» (`20715a3`):** `templates/torture-poc.docx` — **вигаданий** позов (юр-сенсу нема → ревʼю Олі не треба → можна робити максимально підлим), кодує 10 гоч: сусідні розгалуження · голий `{% if flag %}` · вкладений if в if/else · `{%tr%}` в окремих контрол-рядках · presence-гард на опційному полі · склонення жін.(незмінне прізвище)+чол. · дати прописом · змінна в колонтитулі · `keep_with_next`. **Усі 10 проходять.** Обидва сценарії → `%PDF-1.7` через живий Gotenberg, 0 HITL-флагів.
- **✅ Закрито реальну діру:** `extract.derived_sources` ОГОЛОШУВАВ карту вычисляемых ключів, але ніхто її не ВИКОНУВАВ (s85 будував контекст ad hoc у scratchpad) → карта і рушій могли розʼїхатись мовчки, збій упав би на юриста, не на CI. → **`src/doc_engine/context.py`** + parity-тест.
- **✅ Правило опційних полів = структурна відповідь на знахідку A демо D7:** required без відповіді → ключ ВІДСУТНІЙ → preflight падає гучно; optional → `""` → гард ховає блок. Доведено: `________` немає ніде крім підпису, який юрист намалював сам.
- **🐛 Баг класу «юрист не побачить»:** `date_words()` вже вшиває «року» → `{{ x_words }} року народження` друкує **«...1988 року року народження»**. У рушії A `formatDate` дає `03.11.1988`, де фраза коректна. Юрист бачить лише ІМʼЯ поля → пастка невидима. **Вимога до надстройки: превʼю ЗНАЧЕННЯ, а не назви.**
- **⚠️ Власне припущення СПРОСТОВАНО фактом:** гейт гардів НЕ забороняє presence-гард на опційній даті (вычисляемые ключі свідомо звільнені). Натомість доведено головне: **опечатка в гарді** (`has_contarct`) валиться Gate 2 як `unmatched-field` → абзац не може мовчки зникнути з судового документа.
- **✅ Каркас спайка (`1e72ea4`): `apps/word-addin`** — манифест валідний (Word 2019+/M365 Windows), dev-сертифікат довірено, HTTPS :3001 → 200, sideload у реєстрі, Word запущено з копією `torture-spike.docx`. `test_torture.py` приймає `TORTURE_TPL=<шлях>` → той самий сюїт = доказ round-trip.
- **❌ Діра, названа чесно:** опубліковану з Word послугу **в проді рендерити нема чим** — `app.py` (FastAPI) НЕ написаний, Gotenberg лише локальний, тяги шаблону зі Storage нема. Ітерація 1 живе локально; критерій «чистий текст → PDF» доводиться **локально** — так і називати.
- **⚠️ Спірний факт — не тягнути у спеку:** Explore-агент стверджує, що `divorce`/`alimony` досі `generation_mode='js'` і без MCP-тула — суперечить живому демо D7. Перевірити: `select slug, generation_mode, status from services`.

**🔴 НАСТУПНИЙ КРОК — СПАЙК, потрібні руки Сергія** (Claude не клікає у Word): Вставка → Мої надбудови → Developer Add-ins → «Legal AI (spike)» → кнопки згори вниз → **Ctrl+S** → `TORTURE_TPL=<копія> python -m pytest tests/test_torture.py`. **Найстрашніше невідоме — перша ж кнопка:** чи вціліє **CDATA** після збереження у Word (`payload form: cdata` vs `escaped`); `escaped` = `docx_meta.py` більше не бачить блоб → формат треба міняти.
**Далі (порядок у спеці `/feature-spec word-addin-authoring`, Tier 2):** Фаза 1 «Компілятор» (контракт → **канал схеми** → generic-роутер → цитати→watched_laws) — **доводиться БЕЗ Word взагалі**, офлайн, на `alimony-poc.docx` → Фаза 2 «Надстройка» → Фаза 3 «Публікація» (ендпоінт **insert-only, 409 на існуючий slug** — єдине, що стоїть між Word і живим каталогом).
**8 рішень /interview:** повний цикл до публікації · веб = дзеркало · рушій A заморожено як legacy · свій ендпоінт + Supabase Auth, гейти на сервері · `.docx` у приватний Storage-bucket · Word Desktop Windows, sideload лише в Сергія · без AI в панелі (AI = фаза 2) · **амбіція = послуга з одного .docx, ноль коду**.
**🪤 Тулінг:** порт **3001** (3000 = Gotenberg) · манифест валідний лише при `Version >= 1.0` + ОБИДВА icon-елементи · `office-addin-dev-certs install` **БЕЗ `--machine`** · `office-addin-debugging --document` **ІГНОРУЄТЬСЯ** → документ відкривати окремо. **Гоча «IDE перемикає гілку» спрацював ЗНОВУ** одразу після коміту — врятувало «комітити кожну одиницю одразу»; далі працював через `git worktree`, бо в основному дереві лежала НЕзакомічена робота паралельної сесії.
