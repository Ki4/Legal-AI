...
**🟢 SESSION 88 (2026-07-18) — РЕВІЗІЯ ГІЛОК + запис планів/досліджень у main. Гілка `docs/record-plans-research` → merge у main. Код НЕ чіпали.**
- **Привід:** зібрати всі запушені + локальні гілки, вирішити що безпечно мержити, а що = експеримент/дослідження. Правило Сергія: **перевірене та ідеї/плани/дослідження → в main як документи; непроверений КОД → далі перевіряємо в окремих гілках.**
- **Група A (docs) ЗМЕРЖЕНА в main** (`071624a..d24470d`): 3 чисті гілки (`docs/business-model-med-fop`, `docs/document-automation-guides`, `claude/medical-vertical-complexity-scoring`) — `--no-ff`; 2 «конфліктні» (`excel-registry-analysis-plan`, `healthcare-service-package`) — внесено **лише документи** (їхній конфлікт був ТІЛЬКИ у `changelog/session-summary`, бо гілки старі; дослідження — чисте додавання).
- **⚠️ Виправлено власне твердження фактом (`merge-tree` dry-run):** «вся група A мержиться чисто» = НЕВІРНО; 3 чисті, 2 конфліктували лише по службових файлах.
- **Група B — вирішено:** цілком не мержили (обидві гілки правлять `IMPROVEMENTS.md`, коллізія номерів). Перенесено в main новими записами: **#106 Groq deprecation (🟠 дедлайн decommission 2026-08-16)**, #107 пакети документів, #108 context-hygiene v2, #109 Headroom. «Категорії послуг» пропущено — вже є в main як #103.
- **Група C — вирішено:** `word-to-service-poc` (superset) і `doc-engine` (предок) **НЕ мержимо і НЕ видаляємо** — це активний трек верифікації. У main записано лише ПЛАН/ІДЕЮ: новий `docs/architecture/word-to-service-compiler-plan-2026-07.md` (теза «білдер=компілятор», feasibility-вердикт, чесна таблиця статусу верифікації, фазовий шлях у main) + `word-addin-feasibility-2026-07-15.md` + `cabinet-vision.md`/`.html` + `specs/features/template-doc-engine/` + gdpr-бриф + uadopomoga-дослідження (були untracked, тепер збережені).
- **`med-vertical-market-brief` (локальна) — дубль** уже влитого licence-registry аналізу (той самий `ЛР-10.04.2026.xlsx`, ≈35k ФОП/$615k ARR) → в main НЕ тягнули.
- **Локальні незапушені:** `docs/cabinet-vision` — контент тепер у main → можна видалити; `research/gdpr-med-brief` — бриф у main → можна видалити; `med-vertical-market-brief` — дубль, видалити або лишити як історію.
- **🔴 НАСТУПНИЙ КРОК (непроверене — у гілці `feat/word-to-service-poc`):** прогнати СПАЙК Word Add-in (руки Сергія — Claude не клікає Word). Перша перевірка = чи вціліє **CDATA** після Ctrl+S (`cdata` vs `escaped`). Потім Фаза 1 «Компілятор» (офлайн, без Word) → мерж у main першою.
- **⚠️ Відкриті рішення Сергія:** (1) **скіл-на-послугу vs один generic-роутер** (session 87 каже per-service скіл = баг); (2) HTML-preview ще не побудований (#100/#101); (3) прод-рендер (`app.py`/Gotenberg/Storage) — дыра.
