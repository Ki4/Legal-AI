...
> Updated: 2026-06-21 (session 43 — divorce #67 Variant B: майно/борги без порожніх плейсхолдерів. Реалізовано на гілці `fix/divorce-property-debt-variant-b` (`928b5fe`+`4509898`), **НЕ змержено / НЕ задеплоєно** — чекає wording sign-off Ольги. Усі тести зелені.)
> Прочитай эту секцію першою — вона найсвіжіша.

---

## 🆕 Session 43 (2026-06-21) — divorce #67: майно/борги без порожніх плейсхолдерів (Variant B)

### Головне — стан ЗАРАЗ
- **Гілка `fix/divorce-property-debt-variant-b`** (2 коміти: `928b5fe` фікс, `4509898` docs). **НЕ змержено в main, НЕ задеплоєно.** main лишився чистий на `1856c15`.
- **Gate:** тримається до **wording sign-off Ольги** (issue #67, checklist item 1). Після цього: merge + `node scripts/upload-document-template.mjs divorce` + live form-submit smoke.
- **Тести: root vitest 1110/1110 ✅ · client 186/186 ✅ · `tsc -b` clean.**
- **1 сесія = 1 фокус:** лише #67.

### Що зроблено (Variant B — деталі майна/боргів = окремий позов, divorce-документ їх НЕ друкує/НЕ ділить)
1. **Тіло `divorce.document.txt`:** майно `property_dispute='none'` → констатація без поділу/прочерку («…Спір щодо його поділу між сторонами відсутній; вимога про поділ майна не є предметом цього позову.»; гілка `separate` без змін). Борги → констатація без розподілу/прочерку/ст.65.
2. **ПРОШУ:** прибрано пункти поділу майна + розподілу боргів (суд не присуджує поділ неконкретизованого). Нумерація спростилась: діти(5)→аліменти(5/6)→графік(5/6/7); складні if-ланцюжки майна/боргів видалені.
3. **Додатки:** прибрано рядки правовстановлюючих документів на майно + кредитних договорів.
4. **Цитати:** ст.65 СК (єдина згадка = вимога розподілу боргів) прибрана з `divorce.citations.json` + `citations.test.mjs`.
5. **Паритет:** legacy-білдер `divorce-document.js` оновлено дзеркально (263 parity зелені); golden `scenario-3.txt` регенеровано (engine===builder звірено перед записом). `serviceAnatomy`: divorce тепер `unmatched===[]`.
6. **IMPROVEMENTS:** #4с позначено ✅ РЕЗОЛЮЦІЯ (#67); заведено **#85** — авто-визначення суду за адресою (територіальна підсудність; data-проєкт, 🔵, окремо від #67, ідея Сергія).

### Артефакти для Ольги (Сергій зберіг окремо)
- Записка з 3 питаннями (констатувати майно при «спір відсутній»? формулювання боргів? прибрати додатки?) + повний приклад згенерованого позову «до/після» (кейс Шевченко: дитина+аліменти+майно+борги). Згенеровано рендером, не вручну.

### 🔴 Наступний крок (нова сесія)
1. **Зібрати фідбек Ольги** по формулюваннях #67 → за результатом скоригувати текст (зачепить parity/golden ще раз) → **merge + deploy** (`upload-document-template.mjs divorce` + live smoke).
2. ⏰ Backlog Ольги (~2026-06-25, ~через 4 дні): флип `alimony-change`→active, 2 зміни законів (СК/ЦПК), CRON `schedule:`, sign-off `exception_if`.
3. По north-star: збирати реальні заявки/коментарі через адмінку-дзеркало → закономірності → білдер (IMPROVEMENTS #84).

### Запуск середовища
- Тести: `npx vitest run` (root) + `cd apps/client && npx vitest run`. Typecheck: `cd apps/client && npx tsc -b`.
- Деплой шаблону (ПІСЛЯ sign-off): `node scripts/upload-document-template.mjs divorce`.
