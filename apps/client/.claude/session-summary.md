...
**🟢 SESSION 81 (2026-07-08) — Стратегія, БЕЗ змін продукту/коду. Гілка `claude/excel-registry-analysis-plan-jx64xc`. Перецентрування пітча на продукт `Medical License Assistant` (підписка) за матеріалами Олі.**
- **Оля прислала готову концепцію продукту MLA** (docx: 10 функціональних розділів + техспек Telegram-бота + візуальні мокапи). Це підписковий (2–3 тис. грн/міс) B2B-комплаєнс-асистент для медліцензіатів: генерація + **постійна актуалізація** обов'язкових документів (ліцензійні/кадрові/пацієнтські), готовність до перевірок 24/7, захист від позовів і анулювання.
- **Поправка ринку (Оля):** живих ліцензіатів **~17,7 тис.** (реально працюють через ЕСОЗ/eHealth), а НЕ 37,6 тис. з реєстру (той включає непрацюючих). Підтверджує мою вчорашню оговорку. Розбивки по спеціальностях в Олі нема — беремо пропорцію з реєстру (стоматологія ~55%). Використовувати 17,7к як honest live-SAM.
- **Чому підписка сильніша за разовий продаж (і б'є WTP):** Оля формулює механізм проти «лікарі не купують, а позичають документи» — особистий кабінет + щомісячні оновлення під зміни закону/судпрактики. **Цей рекурентний механізм = те, що наш law-monitor уже робить live** → серце підписки вже побудовано. Ров = експертиза Олі (постійно актуалізується) + HITL. Економіка: 17,7к × 2,5к/міс × 12 ≈ TAM ~530 млн/рік; SOM рік 1 (1–3%) ≈ 5–16 млн/рік ARR (📋).
- **Зроблено (docs, git):** концепт-документ `docs/strategy/2026-07-08-medical-license-assistant-concept.md` (10 розділів Now/Next/Later, підписка+економіка, ров, що вже є → маппінг, план пілоту WTP); перецентрована дека `2026-07-08-techlead-pitch.{html,md}` (10 слайдів, MLA headline, live-верифікована Chromium); аддендум-поправка 17,7к у market-analysis. Вчорашня дека `2026-07-07-*` (движок→вертикалі) лишена як варіант.
- **Наступне:** MVP MLA (розділи 2/3/9 генератор+пацієнти+куточок на рушії + sign-off Олі + білінг підписки + кабінет) → пілот на клініках Олі; GDPR go/no-go по медданих ПЕРЕД зберіганням даних пацієнтів (hard-блокер).

**🟢 SESSION 80 (2026-07-07) — Ресерч+стратегія, БЕЗ змін продукту/коду. Гілка `claude/excel-registry-analysis-plan-jx64xc` (docs/research + docs/strategy + scripts, НЕ в main). (1) Порахована медвертикаль з реєстру МОЗ; (2) зібрано пітч-дек для техліда з фокусом на WTP.**
- **Медвертикаль порахована (реєстр ліцензій МОЗ, вигрузка 10.04.2026):** детермінований Python-пайплайн `scripts/lr-registry/` (дедуп рядків-рішень до субʼєкта, типізація ФОП/приватні/комунальні, ICP-сегменти, reconciliation з assert). ✅ **45 315 активних ліцензіатів** (звірка з Опендатабот 43 939 кін.2025, +3,1%), **37 613 ICP** (ФОП+малі приватні), 55% зі стоматологією. Дедлайн МОЗ 08.07.2026 (постанова №781) + порожня ніша між шаблоном ₴900 і юристом ₴3000+ = драйвер. Звіт `docs/research/medical-license-market-analysis-2026.md` (TAM≈74/SAM≈71/SOM 0,7–2,1 млн грн — 📋 гроші на допущеннях), продуктова матриця 36 SKU з чек-листа МОЗ `…-product-matrix-2026.md`, сегментований Excel (лід-листи) відданий Сергію. 🪤 pandas-3.0 `astype(str)` зберігає NaN → ловив 20 ФОП + 2727 рядків-сиріт; пофікшено, reconciliation сходиться точно.
- **Пітч-дек для техліда (`docs/strategy/2026-07-07-techlead-pitch.html` + md-двійник):** клон scroll-snap каркаса деки Олі, 10 слайдів, live-верифіковано в Chromium (light+dark, nav, 10/10 dots). Нарратив «рушій: сімейка доведена → медицина next». **Тон чесний:** ключове питання техліда «чи готові платити» = головна невалідована гіпотеза репо (WTP-множник 0,4; custdev 0/8–12; оплата=заглушка). Дек прямо кладе WTP на стіл + дає дешевий план валідації (A: $0 8–12 custdev через Олю → B: €300–500 Meta smoke з передоплатою → C: Telegram Payments = перша ₴) з go/no-go, і кличе техліда со-власником валідації + закрити payment-loop. Guardrails: НЕ клонувати застарілі full-AI доки; ціни (₴349/₴300–800/медпакети) = гіпотези, не факт продажів.
- **Стан продукту (з розвідки, підтверджує попередні сесії):** наскрізний PoC, 2 live template-послуги, оплата=заглушка («найбільша MVP-діра»), спрос на категорію доведено твердо, **WTP self-service — невалідовано** (custdev не проведено, smoke не запускався). Медвертикаль під GDPR-блокером + sign-off Олі.
