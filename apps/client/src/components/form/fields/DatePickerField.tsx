...
import { parseYMD, formatDisplay, getDaysInMonth, getFirstDayOfWeek, maskDateInput, parseDisplay } from './dateInput'
...
export function DatePickerField({ id, value, onChange, placeholder = 'ДД.ММ.РРРР' }: Props) {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- mirroring an external `value` prop into local edit state; not a derived-state cascade
