...
## Контракт форма↔шаблон: похідний реєстр змінних + гейти на 4 ребрах; збережуваний реєстр відхилено (session 72, #88)

**Контекст.** Структурні діри `________` (шаблон посилається на `{{поле}}`, якого форма не дасть ніколи) можливі, бо `form_config` і `document_template` — два артефакти з неявним контрактом «збіг рядків». Клас проблеми став продуктовим у момент появи редактора юриста (s65–70): доти контракт охороняли git+CI+parity-тести (dev-only правки), self-service переніс охорону всередину продукту. Прецедент — #86 (16 дір у проді через дрейф deploy-шляху). Попереду медвертикаль (~9 простих template-послуг M1–M11) — масштаб каталогу ×5.

**Рішення (Сергій, session 72).**
**1. Correct-by-construction = похідний реєстр (view), НЕ збережуваний.** Істина лишається де була: `form_config` — для полів (його читають TWA+n8n у рантаймі), рушій (`PROVIDED_CONTEXT`/`DERIVED_SOURCES`) — для computed/ai. «Реєстр змінних» = їх union, який `VariablePalette` вже сьогодні обчислює. Окрема таблиця `service_variables` відхилена: другий источник істини про той самий факт («поле існує») → або міграція грошового шляху, або клас sync-багів реєстр↔form_config — відтворення drift-проблеми поверхом вище.
**2. Безпека — гейти на всіх 4 ребрах контракту** (red-team сесії 72 довів, що ребро одне ≠ захист): публікація шаблону · «Відновити» ревізію · збереження форми · флип `generation_mode` (останнє — процедурно, runbook). Інваріант: **опублікована пара завжди консистентна**; чернетка МАЄ право бути невалідною (авторинг). Політика: `apps/client/.claude/reports/2026-07-04-publish-gate-policy.md` (v2, adversarially verified).
**3. Одна траєкторія, три слайси, без роботи двічі:** (1) гейти v2 + dead-ref-предикат 4 класів + parity-тест дзеркало↔рушій — валідаційне ядро (~90% переживає наступні слайси); (2) registry-дисципліна — inline-підсвітка невідомих змінних у CM тим самим предикатом + reference-guard/guided-rename у FormBuilder; (3) preflight-панель (#10 vision) перед заливом медвертикалі (вікно = #BLOCKER-5 GDPR + sign-off Олі M1–M18), M1 — пілот.
**4. Збережуваний реєстр** повернеться лише з майстром повного білдера (#18/#20 — змінні раніше полів) або кросс-сервісними змінними; гейти тоді лишаються як defense-in-depth.

**Зв'язок:** #86/#88 (publish-safety), s64 («форма і шаблон = одна одиниця деплою»), `service-builder-vision.md` §3/§5 (preflight, шари), #84 (той самий патерн: реєстр = код/похідний SSoT, не БД), медвертикаль M1–M18.
