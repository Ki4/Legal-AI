...
6. **Модель затрат (уточнение от 2026-06-12):** построение графа — это разовая dev-задача, выполняемая Claude Code в обычных сессиях по подписке (API-ключ не нужен): ярус 1 — скрипт без LLM, ярус 2 — извлечение+критика в сессии, результат коммитится как данные с фиксированным extraction-промптом в репо. Платный API остаётся только в runtime (hybrid-секция на документ), и стартовать можно на бесплатном Groq (JSON-mode + enum), подключая Claude API лишь при нехватке качества. Подробности — DECISIONS «GraphRAG-стек» → «Модель витрат».
