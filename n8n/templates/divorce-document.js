...
function buildDivorceDocument(answers, ai) {
  ...
  // Спільне майно (Variant B, #67): поділ/деталі — окремий позов, без плейсхолдерів
  if (isTrue(a.has_joint_property)) {
    if (a.property_dispute === 'separate') {
      ...
      doc += 'За час шлюбу сторонами набуто спільне майно. Спір щодо його поділу між сторонами відсутній; вимога про поділ майна не є предметом цього позову.\n\n';
    }
  }

  // Боргові зобов'язання (Variant B, #67): розподіл — окремий позов
  if (isTrue(a.debt_claim)) {
    doc += "Між сторонами наявні спільні боргові зобов'язання. Вимога про їх розподіл не є предметом цього позову та може бути вирішена за домовленістю сторін або в окремому провадженні.\n\n";
  }
  ...
  // У divorce стягнення = лише аліментна гілка → блок під alimony_claim (#87 / issue #76).
  // NB: це ЧАСТИНА 7, не «п.7 ч.3» (інша норма, вище). Гілка «рахунку немає» — закон допускає для фізособи.
  ...
  // Майно/борги (Variant B, #67): НЕ створюють пунктів ПРОШУ — поділ вирішується окремо.
