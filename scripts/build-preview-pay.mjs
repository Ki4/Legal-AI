 ...
 *                             honest: true only on ok+message_id, false on 403/timeout/opt-out)
 ...
 *     on a confirmed send (ok + message_id), false on a swallowed 403 (user never pressed
...
const FINALIZE_DELIVERY_CODE = [
  ...
  '// state the delivery FACT instead of a present-continuous guess (#89 deferred).',
  '// Honest-by-default: true ONLY on a positive success signal (ok === true AND a',
  '// message_id); false on ANYTHING else — a 403 (user never pressed Start), a block,',
  '// a timeout, an n8n error item, OR the opt-out branch where Send PDF never ran.',
  "// Both 'Send to bot?' outputs converge here; on the opt-out branch $('Send PDF') is",
  '// unexecuted, so it is read defensively (try/catch → treated as no confirmed send).',
  '// Send PDF has onError=continueRegularOutput, so a failed send still reaches here.',
  '// ═══════════════════════════════════════════════════════',
  '',
  'let tg = null;',
  "try { tg = $('Send PDF').first().json; } catch (e) { tg = null; }",
  'const delivered = !!(tg && tg.ok === true && tg.result && tg.result.message_id != null);',
...
const nodes = [
  ...
  },
  {
    ...
    notes: 'Read the Telegram result → delivered_to_bot (honest: true only on ok+message_id). Both Send to bot? branches converge here so Respond OK reports the real delivery fact on either path.',
  },
  {
    id: 'respond-ok-001',
    name: 'Respond OK',
