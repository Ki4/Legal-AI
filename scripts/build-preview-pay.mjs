 ...
 *     → Send to bot?  ─true→ Download PDF → Send PDF ─┐
 *                     ─false→ ───────────────────────┤
 *                        → Finalize Delivery (reads the Telegram result → delivered_to_bot;
 *                             honest: true only on Telegram ok===true, false on 403/timeout/opt-out)
 *                        → Respond OK   ({ signed_url, expires_at, delivered_to_bot })
 *
 ...
 *   - Honest delivery signal (#89 deferred): Finalize Delivery reads the Telegram
 *     sendDocument result and reports `delivered_to_bot` in the paid response — true ONLY
 *     on a confirmed send (Telegram ok===true), false on a swallowed 403 (user never pressed
 *     Start) / timeout / opt-out. The client states the FACT, never a delivery it can't
 *     confirm (court-ready product: no false claims). false = «could not confirm», not
 *     «not delivered».
 *
...
// ─── Finalize Delivery (Code) — derive delivered_to_bot from the Telegram result ─
const FINALIZE_DELIVERY_CODE = [
  '// ═══════════════════════════════════════════════════════',
  '// Finalize Delivery — GENERATED by scripts/build-preview-pay.mjs',
  '// Derive delivered_to_bot from the Telegram sendDocument result so the client can',
  '// state the delivery FACT instead of a present-continuous guess (#89/#90 deferred).',
  '// Honest-by-default: true ONLY on Telegram ok===true — its authoritative success flag',
  '// (a failed sendDocument always returns ok:false or an n8n error item). false on ANY',
  '// other outcome: a 403 (user never pressed Start), a block, a timeout, an error item,',
  "// OR the opt-out branch where Send PDF never ran ($('Send PDF') is read defensively).",
  '// n8n httpRequest may expose the parsed body directly OR nested under .body, so we read',
  '// both (mirror of the same defensive access in Build Response; the live happy-path was',
  '// observed as { ok:true, message_id }, so we do NOT hard-require result.message_id).',
  '// Send PDF has onError=continueRegularOutput, so a failed send still reaches here.',
  '// ═══════════════════════════════════════════════════════',
  '',
  'let raw = null;',
  "try { raw = $('Send PDF').first().json; } catch (e) { raw = null; }",
  '// Unwrap: the Telegram body is either the item itself or under .body.',
  'const tg = raw && raw.ok !== undefined ? raw : (raw && raw.body) || {};',
  'const delivered = tg.ok === true;',
  'return [{ json: { _delivered_to_bot: !!delivered } }];',
].join('\n');
...
const nodes = [
  ...
  {
    id: 'finalize-delivery-001',
    name: 'Finalize Delivery',
    type: 'n8n-nodes-base.code',
    typeVersion: 2,
    position: [2040, 430],
    parameters: { jsCode: FINALIZE_DELIVERY_CODE },
    notes: 'Read the Telegram result → delivered_to_bot (honest: true only on Telegram ok===true). Both Send to bot? branches converge here so Respond OK reports the real delivery fact on either path.',
  },
  {
    id: 'respond-ok-001',
    name: 'Respond OK',
    ...
    parameters: {
      respondWith: 'json',
      responseBody:
        "={{ JSON.stringify({ success: true, signed_url: $('Build Response').first().json._signed_url, expires_at: $('Build Response').first().json._expires_at, delivered_to_bot: $json._delivered_to_bot === true }) }}",
    },
    notes: 'delivered_to_bot comes from Finalize Delivery ($json), which both branches pass through; signed_url/expires_at are node-refs to Build Response (ran on both branches).',
  },
  {
...
const connections = {
  ...
  'Send to bot?': {
    main: [
      ...
      [{ node: 'Finalize Delivery', type: 'main', index: 0 }],
    ],
  },
  'Download PDF': { main: [[{ node: 'Send PDF', type: 'main', index: 0 }]] },
  'Send PDF': { main: [[{ node: 'Finalize Delivery', type: 'main', index: 0 }]] },
  'Finalize Delivery': { main: [[{ node: 'Respond OK', type: 'main', index: 0 }]] },
};
