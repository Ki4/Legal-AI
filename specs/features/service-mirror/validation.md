...
- [x] **2:** `service_notes` міграція + RLS; юрист створює/закриває нотатку; dev бачить інбокс. (session 41, merged)
- [x] **3:** `service_requests` міграція (026) + Storage bucket `service-examples` (приватний, 10 МБ, PDF/DOC/DOCX) + Storage RLS authenticated; сторінка `Заявки` (форма title/опис/закони/файл + інбокс); приклад завантажується в bucket, dev відкриває через signed URL (60s); status open↔done. Чистий хелпер `serviceRequestFile.ts` (валідація типу/розміру + storage-safe ключ) + 11 тестів. (session 42)
  - **⚠️ Перед використанням наживо:** застосувати migration 026 (таблиця + bucket + storage-політики) — до того заявки/завантаження падатимуть.
  - **Перевірка наживо (лишилось ручним):** залогінитись в адмінку → подати заявку з PDF/DOCX → переконатись, що рядок з'явився в інбоксі і «📄 Приклад документа» відкриває signed URL; не-PDF/DOC або >10 МБ → інлайн-помилка.
