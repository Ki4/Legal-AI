...
- [ ] **Агент «що змінилось» (law-change-impact)** 🔴 — поверх CRON-моніторингу: детермінований diff редакцій (Node) + попередня чернетка «що змінилось + вплив по послугах» (n8n/Groq, enum-констрейнт + abstention) у панелі «Зміни законів» для підпису Олі. Закриває єдиний реальний юр-ризик (проґавлена зміна закону). ТЗ: `specs/features/law-change-impact/` (Tier 2, спека готова — чекає реалізації)
