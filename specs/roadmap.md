...
- [~] **Адмінка-дзеркало (service-mirror, #66)** — read-only огляд послуги: форма як є + анатомія документа (поля in/out + health 🟢/🟡/🔴) + закони (цитати + stale/changed badge). Слайс 1 (G1-G3) ✅; коментарі юриста (слайс 2) + заявка на послугу з прикладом (слайс 3) — далі. Спека `specs/features/service-mirror/` 🟡
